...
#   DESCRIPTION:  alias gnu coreutils etc on macos
#        AUTHOR:  Sorin Ionescu (sorin.ionescu@gmail.com), adapted by me .bjs
#       VERSION:  2.0.0
# ------------------------------------------------------------------------------

# Detect if GNU coreutils are installed by looking for gwhoami, or extras with gsed
if [[ ! -x "${commands[gwhoami]}" ]] && [[ ! -x "${commands[gsed]}" ]]; then
  return
fi
...
__gnu_utils() {
  ...
  gcmds+=('gsed' 'gtar' 'gtime' 'gmake' 'ggrep' 'gawk' 'gunits')
